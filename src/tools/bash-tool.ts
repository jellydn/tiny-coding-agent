...
export const bashTool: Tool = {
	...
	async execute(args: Record<string, unknown>): Promise<ToolResult> {
		...
		if (suspiciousChars) {
			...
			console.warn(`[Security] Command contains potentially unsafe metacharacters: ${suspiciousChars.join(", ")}`);
		}
		...
		return new Promise((resolve) => {
			...
			child.on("close", (exitCode: number) => {
				...
				if (killed) {
					resolve({
						success: false,
						error: formatOutput(stdoutStr, stderrStr) || `Command timed out after ${effectiveTimeout}ms`,
					});
					return;
				}
