...
import type { McpConnection, McpToolCallResult, McpToolDefinition } from "./types.js";
...
export class McpClient {
	...
	async connect(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to connect to MCP server "${this._name}": ${error instanceof Error ? error.message : String(error)}`
			);
		}
	}
