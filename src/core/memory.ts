...
import type { Memory, MemoryCategory, MemoryStoreOptions } from "./memory-eviction.js";
import {
	countMemoryTokens,
	evictOldest,
	generateMemoryId,
...
export type { ContextStats, Memory, MemoryCategory, MemoryStoreOptions } from "./memory-eviction.js";
...
export class MemoryStore {
	...
	countTokens(): number {
		return countMemoryTokens(this._memories);
	}
	...
	private _evictIfNeeded(): void {
		while (this._memories.size > 1) {
			const tokenCount = countMemoryTokens(this._memories);

			if (!shouldEvict(this._memories.size, this._maxMemories, tokenCount, this._maxMemoryTokens)) {
				break;
			}

			evictOldest(this._memories, this._sortedIds);
		}
	}
}
