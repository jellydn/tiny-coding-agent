...
Body`;
			...
			expect(() => parseSkillFrontmatter(content)).toThrow("Description must not be empty or whitespace only");
		});
	});
...
Body`;
			...
			expect(() => parseSkillFrontmatter(content)).toThrow("Invalid SKILL.md: missing frontmatter delimiters (---)");
		});
...
Body`;
			...
			expect(() => parseSkillFrontmatter(content)).toThrow("Frontmatter must have a 'description' field");
		});
	});
});
