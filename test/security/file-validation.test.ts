...
describe("write_file security", () => {
	...
	it("should allow writes to safe paths", async () => {
		...
		// Clean up
		await import("node:fs/promises").then((fs) => fs.rm("/tmp/test-file.txt", { force: true }).catch(() => {}));
	});
	...
	it("should allow writes to relative paths without ..", async () => {
		...
		expect(result.success).toBe(true);
		// Clean up
		await import("node:fs/promises").then((fs) => fs.rm("src/test.txt", { force: true }).catch(() => {}));
	});
});
