...
import { OllamaProvider, toErrorMessage } from "../../src/providers/ollama.js";
		...
		it("should yield content chunks during streaming", async () => {
			...
			const stream = new ReadableStream({
				async start(controller) {
					controller.enqueue(
						new TextEncoder().encode(`${JSON.stringify({ message: { content: "Hello" }, done: false })}\n`)
					);
					controller.enqueue(
						new TextEncoder().encode(`${JSON.stringify({ message: { content: " World" }, done: false })}\n`)
					);
