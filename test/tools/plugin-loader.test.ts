...
describe("isTool", () => {
	...
	it("should return false for objects missing required properties", () => {
		...
		expect(isTool({ name: "test", description: "desc", parameters: {}, execute: "not a function" })).toBe(false);
	});

	it("should return false for objects with wrong property types", () => {
		expect(isTool({ name: 123, description: "desc", parameters: {}, execute: () => {} })).toBe(false);
		expect(isTool({ name: "test", description: 123, parameters: {}, execute: () => {} })).toBe(false);
		expect(isTool({ name: "test", description: "desc", parameters: "object", execute: () => {} })).toBe(false);
	});
